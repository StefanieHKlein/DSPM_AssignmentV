...
# get to venue data
venue_content_p0 <- venue_response_DE_p0[["_embedded"]][["venues"]]
...
I then set up a for loop iterating over all venues on the pages 1-610 (excluded first (no. 0) and last page (no. 611)).
...
Lastly, I retrieve the venue data for the last 18 venues.
...
I choose the country 'Spain' (countryCode = "ES").
First, I extract the first 20 venues (page no. 0).
...
I then set up a data frame for the Spanish venues which I fill using a for loop.
...
# create data frame for 20 (venues per page) * 153 (full pages) observations.
...
I visualize the Spanish venue data:
...
# venue_data_total_ES$longitude[venue_data_total_ES$longitude < 5.866944 | 
                           # venue_data_total_ES$longitude > 15] <- NA

# venue_data_total_ES$latitude[venue_data_total_ES$latitude < 47.271679 | 
                        # venue_data_total_ES$latitude > 55.0846] <- NA
